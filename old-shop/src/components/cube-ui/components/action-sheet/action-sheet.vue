<template>
  <transition name="cube-action-sheet-fade">
    <cube-popup type="action-sheet" :class="{'cube-action-sheet_picker': pickerStyle}" :center="false" :mask="true" :z-index="zIndex" v-show="isVisible" @mask-click="maskClick">
        ...
        <div class="cube-action-sheet-panel cube-safe-area-pb" v-show="isVisible" @click.stop>
          ...
          <div class="cube-action-sheet-content">
            <ul class="cube-action-sheet-list">
              <li class="cube-action-sheet-item border-bottom-1px" v-for="(item, index) in data" :key="index" :data-align="item.align" :class="[ item.class || '', index === active ? 'cube-action-sheet-item_active': '' ]" v-html="item.content" @click="itemClick(item, index)"></li>
            </ul>
          </div>
          <div class="cube-action-sheet-space"></div>
          <div class="cube-action-sheet-cancel" @click="cancel">
            <span>{{cancelTxt}}</span>
          </div>
        </div>
      </transition>
